...
class NewsApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'Tech News'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  Future<List<Article>> _getArticles() async {
    http.Response response = await http.get(Uri.parse(
        "https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=a97cbcd6b09e4341964830e6f4b70820"));
    var jsonData = json.decode(response.body);

    List<Article> articles = [];

    for (var article in jsonData["articles"]) {
      Article newArticle = Article(article["author"], article["title"],
